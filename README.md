...
4. 接收计数增加、解析计数不增加时，在 `GripperOTA_HandleQuery` 定位是否进入/返回；
   解析计数增加而电脑没收到回复时，检查 `g_tcp_send_attempt/success/fail`、
   `TxQueueHandle` 和发送缓冲池是否分配/入队失败。
5. 特别核对现场构建的 `ENABLE_DATA_UPLOAD`。当前仓库值为1，但旧发送循环在
   值为0时会把包括OTA回复在内的TxQueue消息丢弃，表现也可能是TCP连接正常而所有
   回复超时。这是需要核对的代码分支，尚未确认为本次实机根因。

本次仅补充上位机前置检查和诊断日志，未以截图推测为依据修改主控或夹爪启动逻辑。
实机根因仍需结合QUERY结果及必要的主控运行状态确认。
...
2026-09-08 共13项测试通过，覆盖当前夹爪 BIN 的布局/CRC、1 KiB接收限制、
...
这些测试不连接真实机器人。本次是上位机更新，无需因添加界面重新编译或烧录
H723/G431；主控和夹爪仍需满足第1节已有的OTA固件前提。
